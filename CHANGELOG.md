...
- **OpenClaw compatibility** — upgraded to OpenClaw `2026.7.1-2` and moved channel settings to `channelConfigs`.
- **WeCom setup** — `appSecret` is documented as the Secret of an authorized self-built WeCom app; callback configuration supplies only Token and EncodingAESKey.
- **Repository metadata** — updated package and repository references for this maintained fork.

### Fixed

- **Inbound recovery** — persist delivery state per customer-service account so unfinished inbound messages can be recovered after a restart.
