...
function isProtected(position, ranges) {
    for (const r of ranges) {
        if (r.start > position)
            return false; // early break — ranges are sorted
...
function findDirective(text, prefix, protectedRanges) {
...
function stripDirective(text, startIdx, endIdx) {
 ...
 */
export function parseWechatLinkDirective(text, protectedRanges) {
...
export function parseWechatLocationDirective(text, protectedRanges) {
...
export function parseWechatMiniprogramDirective(text, protectedRanges) {
 ...
 */
function parseMenuItemDirective(raw) {
...
export function parseWechatMenuDirective(text, protectedRanges) {
 ...
 */
export function buildMsgMenuPayload(menu) {
    let clickAutoId = 0;
    const list = menu.items.map((item) => {
        switch (item.type) {
            case "click": {
                clickAutoId++;
                return { type: "click", click: { id: item.id ?? String(clickAutoId), content: item.content } };
            }
            case "view":
                return { type: "view", view: { url: item.url, content: item.content } };
            case "miniprogram":
                return {
                    type: "miniprogram",
...
export function parseWechatBusinessCardDirective(text, protectedRanges) {
...
export function parseWechatCaLinkDirective(text, protectedRanges) {
...
export function parseWechatRawDirective(text, protectedRanges) {
 ...
 * outside of markdown code blocks, inline code, and blockquotes.
 */
export function hasWechatDirective(text) {
 ...
 */
export function parseWechatDirective(text) {
