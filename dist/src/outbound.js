...
function warnSessionLimit(err, externalUserId, openKfId) {
    if (isSessionLimitError(err)) {
        getSharedContext()?.botCtx.log?.warn(`${logTag()} session limit exceeded (48h/5-msg) for user=${externalUserId} kf=${openKfId}. ` +
...
export const wechatKfOutbound = {
    ...
    sendText: async ({ cfg, to, text, accountId }) => {
        ...
        if (directive.location) {
            try {
                const locResult = await sendLocationMessage(account.corpId, account.appSecret, externalUserId, openKfId, directive.location);
                if (directive.text) {
        ...
        if (directive.menu) {
            try {
                ...
                const menuResult = await sendMsgMenuMessage(account.corpId, account.appSecret, externalUserId, openKfId, menuPayload);
                if (directive.text) {
                    const remainChunks = chunkTextByUtf8Bytes(formatText(directive.text), BYTE_LIMIT);
                    for (const chunk of remainChunks) {
                        await sendTextMessage(account.corpId, account.appSecret, externalUserId, openKfId, chunk);
        ...
        if (directive.businessCard) {
            try {
                const cardResult = await sendBusinessCardMessage(account.corpId, account.appSecret, externalUserId, openKfId, directive.businessCard);
                if (directive.text) {
                    const remainChunks = chunkTextByUtf8Bytes(formatText(directive.text), BYTE_LIMIT);
                    for (const chunk of remainChunks) {
                        await sendTextMessage(account.corpId, account.appSecret, externalUserId, openKfId, chunk);
        ...
        if (directive.caLink) {
            try {
                const caResult = await sendCaLinkMessage(account.corpId, account.appSecret, externalUserId, openKfId, directive.caLink);
                if (directive.text) {
                    const remainChunks = chunkTextByUtf8Bytes(formatText(directive.text), BYTE_LIMIT);
                    for (const chunk of remainChunks) {
                        await sendTextMessage(account.corpId, account.appSecret, externalUserId, openKfId, chunk);
        ...
        if (directive.raw) {
            try {
                const rawResult = await sendRawMessage(account.corpId, account.appSecret, externalUserId, openKfId, directive.raw.msgtype, directive.raw.payload);
                if (directive.text) {
                    const remainChunks = chunkTextByUtf8Bytes(formatText(directive.text), BYTE_LIMIT);
                    for (const chunk of remainChunks) {
                        await sendTextMessage(account.corpId, account.appSecret, externalUserId, openKfId, chunk);
    ...
    sendMedia: async ({ cfg, to, text, mediaUrl, accountId }) => {
        ...
        if (mediaUrl) {
            ...
            try {
                const result = await uploadAndSendMedia(account.corpId, account.appSecret, externalUserId, openKfId, loaded.buffer, loaded.fileName ?? "file", mediaType);
                if (text?.trim()) {
    ...
    sendPayload: async ({ cfg, to, text, accountId, payload }) => {
        ...
        const location = wechatKf?.location;
        if (location) {
            try {
                const result = await sendLocationMessage(account.corpId, account.appSecret, externalUserId, openKfId, location);
                if ((text ?? payload.text)?.trim()) {
                    await sendTextMessage(account.corpId, account.appSecret, externalUserId, openKfId, formatText((text ?? payload.text)));
                }
                return { channel: CHANNEL_ID, messageId: result.msgid, chatId: to };
            }
            catch (err) {
        ...
        const miniprogram = wechatKf?.miniprogram;
        if (miniprogram) {
        ...
        const msgmenu = wechatKf?.msgmenu;
        if (msgmenu) {
            try {
        ...
        const businessCard = wechatKf?.businessCard;
        if (businessCard) {
            try {
                const result = await sendBusinessCardMessage(account.corpId, account.appSecret, externalUserId, openKfId, businessCard);
                if ((text ?? payload.text)?.trim()) {
                    await sendTextMessage(account.corpId, account.appSecret, externalUserId, openKfId, formatText((text ?? payload.text)));
                }
                return { channel: CHANNEL_ID, messageId: result.msgid, chatId: to };
