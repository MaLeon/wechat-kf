...
// ── Cursor recovery and initial-sync protection ──
...
describe("cursor recovery", () => {
  ...
  it("dispatches a recent first message when no cursor exists", async () => {
    const fsp = await import("node:fs/promises");
  ...
  it("saves the latest cursor during an initial sync", async () => {
    const fsp = await import("node:fs/promises");
    (fsp.readFile as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error("no cursor"));
  ...
  it("handles multi-page initial sync correctly", async () => {
    const fsp = await import("node:fs/promises");
    (fsp.readFile as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error("no cursor"));
    ...
    // First initial-sync call should use token (no cursor yet)
    const req1 = mockSyncMessages.mock.calls[0][2];
    expect(req1.token).toBe("sync_token_123");
    expect(req1.cursor).toBeUndefined();

    // Second call should use cursor from first page
    const req2 = mockSyncMessages.mock.calls[1][2];
    expect(req2.cursor).toBe("drain_page1");
    expect(req2.token).toBeUndefined();

    expect(mockRuntime.channel.reply.dispatchReplyFromConfig).toHaveBeenCalledTimes(3);
  });

  it("handles initial sync_msg failure gracefully", async () => {
    const fsp = await import("node:fs/promises");
    (fsp.readFile as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error("no cursor"));
    ...
    expect(logMessages.some((m) => m.includes("sync_msg failed") && m.includes("network timeout"))).toBe(true);
    expect(mockRuntime.channel.reply.dispatchReplyFromConfig).not.toHaveBeenCalled();
  });

  it("uses the saved cursor for processing after initial sync", async () => {
    const fsp = await import("node:fs/promises");

    // First call: no cursor → process the recent message and persist a cursor
    (fsp.readFile as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error("no cursor"));
    mockSyncMessages.mockResolvedValueOnce({
      errcode: 0,
      errmsg: "ok",
    ...
    // The first response cursor is persisted and used by the next sync.
    const secondSyncReq = mockSyncMessages.mock.calls[1][2];
    expect(secondSyncReq.cursor).toBe("existing_cursor");
  ...
  it("does not drain when cursor exists — processes messages normally", async () => {
    ...
    expect(mockRuntime.channel.reply.dispatchReplyFromConfig).toHaveBeenCalledTimes(1);
    expect(logMessages.some((m) => m.includes("sync_msg failed"))).toBe(false);
  });
  ...
  it("processes backlog after a restart when a valid cursor exists", async () => {
    const fsp = await import("node:fs/promises");
    // A valid cursor permits recovery after a short outage.
    ...
    const now = Math.floor(Date.now() / 1000);
    // Two messages are older than the initial-sync threshold.
    ...
    expect(mockRuntime.channel.reply.dispatchReplyFromConfig).toHaveBeenCalledTimes(3);
    expect(logMessages.some((m) => m.includes("skipping stale msg corrupt_cursor_1"))).toBe(false);
  });
});
