  ...
  debouncer = core.channel.debounce.createInboundDebouncer<PreparedMessage>({
    ...
    onFlush: async (items) => {
      ...
      await completeMessages(
        items[0].ctx.stateDir,
        items[0].openKfId,
        items.map((item) => item.msg.msgid),
      );
    },
    onError: (err, items) => {
...
async function recoverPendingMessages(
  ...
  for (const msg of pending) {
    if (!(await claimMessage(ctx.stateDir, openKfId, msg))) continue;
    ctx.log?.info(`${logTag(openKfId)} recovering unfinished msg ${msg.msgid}`);
    await enqueueClaimedMessage(ctx, account, openKfId, msg);
  }
}

async function enqueueClaimedMessage(
  ctx: BotContext,
  account: ResolvedWechatKfAccount,
  openKfId: string,
  msg: WechatKfMessage,
  ...
  try {
    ...
    if (prepared) await getOrCreateDebouncer(ctx).enqueue(prepared);
    else await completeMessages(ctx.stateDir, openKfId, [msg.msgid]);
  } catch (err) {
